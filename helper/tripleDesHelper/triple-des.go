...
func encrypt(origData, key []byte) ([]byte, error) {
	if len(origData) < 1 || len(key) < 1 {
		return nil, errors.BadRequest("wrong data or key")
	}
	...
	if len(origData)%bs != 0 {
		return nil, errors.BadRequest("wrong padding")
	}
...
func decrypt(crypted, key []byte) ([]byte, error) {
	...
	bs := block.BlockSize()
	if len(crypted)%bs != 0 {
		return nil, errors.BadRequest("wrong crypted size")
	}
...
func TripleDesEncrypt(mode TripleDesMode, data, key []byte) ([]byte, error) {
	...
	return TripleEcbDesEncrypt(data, key)
}
...
func TripleDesDecrypt(mode TripleDesMode, data, key []byte) ([]byte, error) {
	...
	return TripleEcbDesDecrypt(data, key)
}
